...
(function () {
    ...
    const CONFIG = {
        ...
        clearFields: [
            ...
            'RelaisColis-ShippingCharges','RelaisColis-ShippingCharges',
            'SoColissimo-ShippingCharges','SoColissimo-ShippingCharges',
        ],
        ...
        GM_xmlhttpRequest({
            ...
            onload: (res) => {
                try {
                    ...
                    if (!vMatch) return toast('❌ 无法获取版本');
                    const onlineVersion = vMatch[1];
                    if (parseFloat(onlineVersion) > parseFloat(CONFIG.currentVersion)) {
                        toast(`✅ 发现新版本：${onlineVersion}，正在打开更新页面`);
                        setTimeout(() => GM_openInTab(CONFIG.updateCheckUrl), 600);
                    } else {
                        toast('✅ 当前已是最新版本！');
                    }
                } catch (e) {
            ...
            onerror: () => toast('❌ 检查更新失败'),
            ontimeout: () => toast('❌ 检查更新超时')
        });
    }
